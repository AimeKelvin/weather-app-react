...
export function WeatherDashboard() {
  ...
  const loadDefaultWeather = async () => {
    ...
    setError(null);
    try {
      const data = weatherApi.getMockWeather();
      setWeatherData(data);
    } catch (err) {
  ...
  return <div className={`min-h-screen w-full py-4 sm:py-8 px-3 sm:px-4 transition-colors duration-500 ${isDarkMode ? 'bg-black' : 'bg-gradient-to-br from-gray-50 via-white to-gray-50'}`}>
      <div className="max-w-6xl mx-auto">
        ...
        duration: 0.6
          ...
          <motion.button onClick={() => setIsDarkMode(!isDarkMode)} whileHover={{
          scale: 1.1
        }} whileTap={{
          scale: 0.9
        }} className={`absolute right-0 top-0 p-2.5 rounded-xl transition-all duration-300 ${isDarkMode ? 'bg-gray-900 hover:bg-gray-800 text-yellow-400' : 'bg-white hover:bg-gray-50 text-gray-700 shadow-md hover:shadow-lg'}`} title={isDarkMode ? 'Light mode' : 'Dark mode'}>
        ...
        <AnimatePresence mode="wait">
          ...
          {error && <motion.div key="error" initial={{
          opacity: 0,
          y: -10
        }} animate={{
          opacity: 1,
          ...
          opacity: 0,
          y: -10
        }} className={`rounded-xl p-3 sm:p-4 flex items-center gap-3 mb-6 sm:mb-8 transition-colors duration-300 ${isDarkMode ? 'bg-red-950 border border-red-900' : 'bg-red-50 border border-red-200'}`}>
              <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5 text-red-600 flex-shrink-0" />
              <p className={`text-sm sm:text-base transition-colors duration-300 ${isDarkMode ? 'text-red-400' : 'text-red-800'}`}>
                {error}
              </p>
            </motion.div>}
          {weatherData && !isLoading && <motion.div key="content" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
        ...
        {!weatherData && !isLoading && !error && <motion.div initial={{
        opacity: 0
      }} animate={{
        opacity: 1
